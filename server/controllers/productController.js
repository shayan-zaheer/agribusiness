...
exports.addProduct = asyncErrorHandler(async (request, response, next) => {
    const { name, description, price, quantityAvailable, category } =
        request.body;
...
exports.getProductById = asyncErrorHandler(async (request, response, next) => {
    const product = await Product.find({_id: request.params.productId}).populate("seller");
    response.status(200).json({
        status: "success",
        data: {
            product
...
exports.showOwnProducts = asyncErrorHandler(async (request, response, next) => {
    const products = await Product.find({ seller: request.params.id }).populate(
        "seller"
    );
    response.status(200).json({
        status: "success",
